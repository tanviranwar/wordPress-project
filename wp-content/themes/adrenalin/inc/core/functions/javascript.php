...
function cg_register_production_js() {
	...
	if ( ! is_admin() ) {
		...
		wp_enqueue_script( 'flexslider', CG_JS . '/src/cond/jquery.flexslider-min.js', array( 'jquery' ), '', true );

		if ( $cg_product_quick_view == 'enabled' ) {
			wp_enqueue_script( 'wp-util' );
			wp_enqueue_script( 'cg_quickview', CG_JS . '/src/cond/cg_quickview.js', array( 'jquery' ), '', true );
			wp_localize_script( 'cg_quickview', 'cg_ajax', array(
				'cg_ajax_url' => admin_url( 'admin-ajax.php' ),
			) );
		}
